...
def get_data(json_name,augment_num):
    ...
    #print("Data Dic Len : ",len(data_dic))
    data_dic_name_list = []
    ...
    #print("data_dic_name_list Len : ",len(data_dic_name_list))
    print('finish loading')
    return data_dic_name_list,data_dic


class DINetDataset(Dataset):
    def __init__(self,path_json, path_json_org, augment_num,mouth_region_size, n=720):
        super(DINetDataset, self).__init__()
        self.data_dic_name_list,self.data_dic = get_data(path_json,augment_num)
        self.data_dic_name_list_org,self.data_dic_org = get_data(path_json_org,augment_num)

        #self.data_dic_name_list = self.data_dic_name_list  + self.data_dic_name_list_org 
        #self.data_dic = self.data_dic + self.data_dic_org
    ...
    def __getitem__(self, index):
        ...
        if chance > 0.7:
            data_dic_name_list = self.data_dic_name_list_org
            data_dic = self.data_dic_org
            print(chance," HDTF")

        else:
            data_dic_name_list = self.data_dic_name_list
            data_dic = self.data_dic
            print(chance,"USLU")

        video_name = data_dic_name_list[index]
        #print("Video name : ",video_name)
        video_clip_num = len(data_dic[video_name]['clip_data_list'])
        #print("video clip num : ",video_clip_num)
        source_anchor = random.sample(range(video_clip_num), 1)[0]
        #print("source_anchor : ",source_anchor)
        source_image_path_list = data_dic[video_name]['clip_data_list'][source_anchor]['frame_path_list']
        ...
        for source_frame_index in range(2, 2 + 5):
            ...
            deepspeech_array = np.array(data_dic[video_name]['clip_data_list'][source_anchor]['deep_speech_list'][
            ...
            for reference_anchor in reference_anchor_list:
                reference_frame_path_list = data_dic[video_name]['clip_data_list'][reference_anchor][
                    'frame_path_list']
        ...
        deep_speech_full = np.array(data_dic[video_name]['clip_data_list'][source_anchor]['deep_speech_list'])
