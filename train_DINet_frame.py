...
if __name__ == "__main__":
    ...
    for epoch in range(opt.start_epoch, opt.non_decay+opt.decay+1):
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            writer.add_scalar("Loss_DI/freeze_dI", float(loss_dI), epoch)
            writer.add_scalar("Loss_GI/freeze_dI", float(loss_g_dI), epoch)
            writer.add_scalar("Loss_perception/freeze_dI", float(loss_g_perception), epoch)
            writer.add_scalar("Loss_G/freeze_dI", float(loss_g), epoch)
        ...
        writer.add_images("generated/1-Input", source_image_data, global_step=epoch)
        writer.add_images("generated/3-Fake", fake_out, global_step=epoch)
